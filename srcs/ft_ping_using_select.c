...
)
{
	...
	return (now.tv_sec >= _ctx->options.timeout);
}
...
)
{
	...
	t_ssize recv_len = recvmsg(_ctx->socket, &msg, 0);
...
)
{
	...
	while (g_stop == false)
	{
		...
		fd_set fdset;
		FD_ZERO(&fdset);
		FD_SET(_ctx->socket, &fdset);

		// Note: Required to be able to handle `interval`, `timeout`, and `linger`
		t_int_32 n = select(_ctx->socket + 1, &fdset, NULL, NULL, &resp_time);

		if (n < 0)
		{
			if (errno != EINTR)
			{
				dprintf(2, "ft_ping: select failed: %s\n", strerror(errno));
				ft_exit(FT_EXIT_ERR);
			}
		...
		else if (n == 1)
		{
			...
			if (_is_timeout_reached(_ctx) || (_ctx->options.count && (_ctx->stats.received + _ctx->stats.duplicated + _ctx->stats.failed) >= _ctx->options.count))
			{
				break;
			}
		}
		...
		{
			if (!_ctx->options.count || _ctx->stats.transmitted < _ctx->options.count)
			{
				_ping_transmit(_ctx);
...
)
{
	...
	for (i = 1; i < _argc; i++)
	{
		...
		for (j = 0; j < FT_ARRAY_SIZE(opts); j++)
		{
			...
			t_size prefix_len = ft_str_length(opt->prefix);
