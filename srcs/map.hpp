...
#include "utility.hpp"
#include "rb_tree.hpp"
#include "multimap.hpp" // Note: Not used in this file, but required to be defined in the same header file
 ...
 * @ref https://cplusplus.com/reference/map/map/
 */
template<
	typename Key,
	typename T,
...
private:
	typedef ft::map< Key, T, Compare, Allocator >  _self_type;
...
public:
	...
	// - Access ---------------------------
	mapped_type &       at( key_type const & key )       {       iterator it = this->find( key ); if ( it == this->end() ) { throw ft::out_of_range( "map::at" ); } return it->second; }
	mapped_type const & at( key_type const & key ) const { const_iterator it = this->find( key ); if ( it == this->end() ) { throw ft::out_of_range( "map::at" ); } return it->second; }

	// - Lookup ---------------------------
	size_type count( key_type const & key ) const { return ( this->_container.find( key ) != this->_container.end() ); }
...
public:
	...
	friend bool
	operator==(
		ft::map< _Key, _T, _Compare, _Allocator > const & x,
		ft::map< _Key, _T, _Compare, _Allocator > const & y
	);
...
bool
operator==(
	ft::map< Key, T, Compare, Allocator > const & x,
	ft::map< Key, T, Compare, Allocator > const & y
)
{
 ...
 * Swap
 *
 * @ref https://cplusplus.com/reference/map/map/swap-free/
 * @ref https://en.cppreference.com/w/cpp/container/map/swap2
 */
template<
	typename Key,
	typename T,
...
void
swap(
	ft::map< Key, T, Compare, Allocator > & x,
