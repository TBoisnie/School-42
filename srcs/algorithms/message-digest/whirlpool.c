...
{ \
	b =                             t[(a[ n         ] >> 56) & 0xFF]     ; \
	b ^= FT_MEM_BIT_ROTATE_RIGHT_64(t[(a[(n + 7) % 8] >> 48) & 0xFF],  8); \
	b ^= FT_MEM_BIT_ROTATE_RIGHT_64(t[(a[(n + 6) % 8] >> 40) & 0xFF], 16); \
	b ^= FT_MEM_BIT_ROTATE_RIGHT_64(t[(a[(n + 5) % 8] >> 32) & 0xFF], 24); \
	b ^= FT_MEM_BIT_ROTATE_RIGHT_64(t[(a[(n + 4) % 8] >> 24) & 0xFF], 32); \
	b ^= FT_MEM_BIT_ROTATE_RIGHT_64(t[(a[(n + 3) % 8] >> 16) & 0xFF], 40); \
	b ^= FT_MEM_BIT_ROTATE_RIGHT_64(t[(a[(n + 2) % 8] >>  8) & 0xFF], 48); \
	b ^= FT_MEM_BIT_ROTATE_RIGHT_64(t[(a[(n + 1) % 8] >>  0) & 0xFF], 56); \
	b ^= c; \
}
...
struct s_ctx
{
	union
	{
		...
		t_qword chunk_64[CHUNK_SIZE / sizeof(t_qword)];
	};
	...
	union
	{
		t_byte  state_8 [WHIRLPOOL_DIGEST_SIZE];
		t_qword state_64[WHIRLPOOL_DIGEST_SIZE / sizeof(t_qword)];
	};
...
static t_void
_whirlpool_transform(
	t_ctx * _ctx
)
{
	static t_qword const rc[10] =
	{
		0x1823C6E887B8014F,
		0x36A6D2F5796F9152,
		0x60BC9B8EA30C7B35,
	...
	for ( i = 0 ; i < 8 ; i++ )
	{
		_ctx->chunk_64[i] = ft_sys_to_big_endian_64(_ctx->chunk_64[i]);
	}
	...
	m[0] = _ctx->chunk_64[0] ^ k[0];
	m[1] = _ctx->chunk_64[1] ^ k[1];
	m[2] = _ctx->chunk_64[2] ^ k[2];
	m[3] = _ctx->chunk_64[3] ^ k[3];
	m[4] = _ctx->chunk_64[4] ^ k[4];
	m[5] = _ctx->chunk_64[5] ^ k[5];
	m[6] = _ctx->chunk_64[6] ^ k[6];
	m[7] = _ctx->chunk_64[7] ^ k[7];
	...
	_ctx->state_64[0] ^= m[0] ^ _ctx->chunk_64[0];
	_ctx->state_64[1] ^= m[1] ^ _ctx->chunk_64[1];
	_ctx->state_64[2] ^= m[2] ^ _ctx->chunk_64[2];
	_ctx->state_64[3] ^= m[3] ^ _ctx->chunk_64[3];
	_ctx->state_64[4] ^= m[4] ^ _ctx->chunk_64[4];
	_ctx->state_64[5] ^= m[5] ^ _ctx->chunk_64[5];
	_ctx->state_64[6] ^= m[6] ^ _ctx->chunk_64[6];
	_ctx->state_64[7] ^= m[7] ^ _ctx->chunk_64[7];
}
...
)
{
	...
	{
		n = FT_MATH_MIN(_data_length, CHUNK_SIZE - _ctx->chunk_length);
...
)
{
	...
	// Message length uses 32 bytes (256 bits) in theory,
	// but 8 bytes (64 bits) in practice
	...
	{
		0, 0, 0, ft_sys_to_big_endian_64(_ctx->size * 8)
	};

	_whirlpool_update(_ctx, padding, padding_size);
	_whirlpool_update(_ctx, (t_byte const *) data_size_in_bits, MESSAGE_LENGTH_SIZE);

	#undef MESSAGE_LENGTH_SIZE

	t_size i;

	for ( i = 0 ; i < FT_ARRAY_SIZE(_ctx->state_64) ; i++ )
	{
		_ctx->state_64[i] = ft_sys_to_big_endian_64(_ctx->state_64[i]);
	}

	ft_mem_copy(_ctx->state_8, _digest, WHIRLPOOL_DIGEST_SIZE);
...
t_void
whirlpool_memory(
	t_byte _digest[/* WHIRLPOOL_DIGEST_SIZE */],
	t_byte const * _mem,
	t_size _size
)
{
	...
	_whirlpool_init(&ctx);
	_whirlpool_update(&ctx, _mem, _size);
	_whirlpool_final(&ctx, _digest);
}
...
)
{
	...
	{
		_whirlpool_update(&ctx, buffer, length);
	}
