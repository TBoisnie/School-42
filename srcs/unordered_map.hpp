...
#include "unordered_multimap.hpp" // Note: Not used in this file, but required to be std compliant.
 ...
 * UnorderedSet
 *
 * @ref https://cplusplus.com/reference/unordered_map/unordered_map/
...
class unordered_map
{
	// Type --------------------------------------------------------------------
private:
	typedef ft::unordered_map< Key, Value, Hash, Equal, Allocator >  _self_type;
...
public:
	...
	)
	{
		...
		{
			position = this->insert( value_type( key, mapped_type() ) ).first;
		}
...
public:
	...
	// - Access ------------------------
	mapped_type &       at( key_type const & key )       {       iterator it = this->find( key ); this->_check_range( it ); return it->second; }
	mapped_type const & at( key_type const & key ) const { const_iterator it = this->find( key ); this->_check_range( it ); return it->second; }
...
private:
	...
	) const
	{
		if ( it == this->end() )
		{
			throw ft::out_of_range("unordered_map::_check_range");
		}
	}
...
public:
	...
	bool
	friend operator==(
		ft::unordered_map< _T, _Hash, _Equal, _Allocator > const & x,
		ft::unordered_map< _T, _Hash, _Equal, _Allocator > const & y
	);
...
bool
operator==(
	ft::unordered_map< T, Hash, Equal, Allocator > const & x,
	ft::unordered_map< T, Hash, Equal, Allocator > const & y
)
{
 ...
 * Swap
 *
 * @ref https://cplusplus.com/reference/unordered_map/unordered_map/swap(global)/
 * @ref https://en.cppreference.com/w/cpp/container/unordered_map/swap2
 */
template<
	typename T,
	typename Hash,
...
void
swap(
	ft::unordered_map< T, Hash, Equal, Allocator > & x,
	ft::unordered_map< T, Hash, Equal, Allocator > & y
)
{
	x.swap( y );
}
