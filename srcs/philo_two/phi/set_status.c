...
/*   Updated: 2020/12/04 23:36:20 by tboisnie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		phi_set_status(t_config *c, size_t pos, t_phi_status s, int lock)
{
	...
	if (lock && sem_wait(c->write_locker))
	{
		ret_error(0, "Error: phi_status - Sem wait failed.\n");
		return ;
	}
	if (c->dead > 0)
	{
		if (lock && sem_post(c->write_locker))
			ret_error(0, "Error: phi_status - Sem post failed.\n");
		return ;
	}
