...
/*   Updated: 2020/12/05 20:20:19 by tboisnie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	s_live(void *args_v)
{
	...
	if (pthread_join(args->c->death_monitors[args->pos], NULL))
		return (ret_error(1, "Error: death_m - pthread join failed.\n"));
	exit(args->c->dead);
}
...
int			phi_live_start(t_config *c, t_args *args)
{
	...
	{
		if ((c->threads[i] = fork()) == -1)
			return (ret_error(1, "Error: phi_live - fork failed.\n"));
		else if (c->threads[i] == 0)
		{
			return (s_live(&args[i]));
		}
		usleep(25);
		i += 2;
	}
...
int			phi_live_wait(t_config *c)
{
	...
	{
		ret = 0;
		if ((error = waitpid(-1, &ret, 0)) == -1)
			ret_error(1, "Error: phi_live - Waitpid failed.\n");
		if (error == -1 || (WIFEXITED(ret) && WEXITSTATUS(ret)))
		{
			i = 0;
			while (i < c->number_of_philosopher)
			{
				kill(c->threads[i], SIGKILL);
				++i;
			}
			break ;
		}
