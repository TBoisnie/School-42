...
class Lexer(Iterable[Token]):
	...
	def _generateAtom(self) -> Token:
		...
		match self._current_chr:
			# Groups
			case Char.PAREN_L:   token_type = Token.Type.PAREN_L
			case Char.PAREN_R:   token_type = Token.Type.PAREN_R
			case Char.BRACK_L:   token_type = Token.Type.BRACK_L
			case Char.BRACK_R:   token_type = Token.Type.BRACK_R
			# Operators
			case Char.ADD:       token_type = Token.Type.ADD
			case Char.SUB:       token_type = Token.Type.SUB
			case Char.MUL:
				self._next()
				token_type = \
					Token.Type.MUL_MAT if Char.match(self._current_chr, Char.MUL) else \
					Token.Type.MUL
			case Char.DIV:       token_type = Token.Type.DIV
			case Char.MOD:       token_type = Token.Type.MOD
			case Char.POW:       token_type = Token.Type.POW
			case Char.EQUAL:     token_type = Token.Type.EQUAL
			# Punctuations
			case Char.COMMA:     token_type = Token.Type.COMMA
			case Char.SEMICOLON: token_type = Token.Type.SEMICOLON
			case Char.QUESTION:  token_type = Token.Type.QUESTION
			# Default
			case _:              raise InvalidCharacter(self._current_chr)

		if token_type is not Token.Type.MUL:
			self._next()
