...
int main( void )
{
	Test const tests[] = {
		// From subject
		{ .values = { "10&" },     .expected = false },
		{ .values = { "10|" },     .expected = true },
		{ .values = { "11>" },     .expected = true },
		{ .values = { "10=" },     .expected = false },
		{ .values = { "1011||=" }, .expected = true },
		// Custom
		{ .values = { "11!1&0|^" }, .expected = true },
		{ .values = { "1!1&11^=" }, .expected = true },
	};
	...
	for (Test const & test : tests)
	{
		...
		try
		{
			auto actual = ReadySetBoole::eval_formula(test.values[0]);
			std::cout << actual << "\n";
		}
