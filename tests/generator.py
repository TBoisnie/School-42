...
if __name__ == "__main__":
	...
	parser.add_argument("-s", "--solvable", action=argparse.BooleanOptionalAction, help="Forces generation of a solvable/unsolvable puzzle")
	...
	solv = args.solvable if args.solvable is not None else random.choice([True, False])
	sz = args.size

	puzzle = make_puzzle(sz, solvable=solv, iterations=args.iterations)
	...
	for y in range(sz):
		for x in range(sz):
			print("%s " % (str(puzzle[x + y*sz]).rjust(w)), end='')
		print()
