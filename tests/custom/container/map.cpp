...
#define CTN  NS::map< int, std::string >
...
#ifndef PERFORMANCE_MODE
{
	...
	LOG( "- Access -----------------------" );
	{
		...
		int key = k[ Random::GetInt( 0, k.size() - 1 ) ];
		LOG( "ctn.at( " << key << " ) = " << ctn.at( key ) );
		ctn.at( key ) = Random::GetString();
		LOG( "ctn.at( " << key << " ) = " << ctn.at( key ) );

		try
		{
			ctn.at( 4242 ) = Random::GetString();
		}
		catch ( NS::exception const & err )
		{
			LOG( "ctn.at( 4242 ) = " << err.what() );
		}
	}
	{
		LOG( "-- Const" );
		...
		int key = k[ Random::GetInt( 0, k.size() - 1 ) ];
		LOG( "ctn.at( " << key << " ) = " << ctn.at( key ) );
		ILLEGAL( ctn.at( key ) = Random::GetString(); )
		LOG( "ctn.at( " << key << " ) = " << ctn.at( key ) );
