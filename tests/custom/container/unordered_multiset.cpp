...
#define CTN  NS::unordered_multiset< int >
...
#ifndef PERFORMANCE_MODE
{
	...
	LOG( "- Bucket -----------------------" );
	{
		...
		int k = v[ Random::GetInt( 0, v.size() - 1 ) ];
		int index = Random::GetInt( 0, ctn.bucket_count() - 1 );

		LOG( "ctn.bucket( " << k << " ) = " << ctn.bucket( k ) );
	...
	LOG( "- Lookup -----------------------" );
	{
		...
		int k = v[ Random::GetInt( 0, v.size() - 1 ) ];
		LOG( "ctn.count( " << k << " )" << ctn.count( k ) );
		k = v[ Random::GetInt( 0, v.size() - 1 ) ];
		LOG( "ctn.count( " << k << " )" << ctn.count( k ) );
		k = v[ Random::GetInt( 0, v.size() - 1 ) ];
		LOG( "ctn.count( " << k << " )" << ctn.count( k ) );

		k = v[ Random::GetInt( 0, v.size() - 1 ) ];
		LOG( "ctn.find( " << k << " ) = " << *ctn.find( k ) );

		k = v[ Random::GetInt( 0, v.size() - 1 ) ];
		NS::pair< CTN::iterator, CTN::iterator > range = ctn.equal_range( k );
	...
	}
	{
		...
		k = v[ Random::GetInt( 0, v.size() - 1 ) ];
		NS::pair< CTN::const_iterator, CTN::const_iterator > range = ctn.equal_range( k );
		LOG_URANGE( CTN, range.first, range.second );
	}
	LOG( "- Modifier ---------------------" );
	{
		...
		LOG( "-- Insert" );
		ctn.insert( Random::GetInt() );

		it = ctn.begin();
		NS::advance( it, Random::GetInt( 4, 12 ) );

		ctn.insert( it, Random::GetInt() );
		LOG_URANGE( CTN, ctn.begin(), ctn.end() );
		...
		LOG( "-- Erase" );
		ctn.erase( v[ Random::GetInt( 0, v.size() / 2 ) ] );
		LOG_URANGE( CTN, ctn.begin(), ctn.end() );

		ctn.erase( ctn.find( v[ Random::GetInt( v.size() / 2 + 2, v.size() - 1 ) ] ) );
		LOG_URANGE( CTN, ctn.begin(), ctn.end() );
