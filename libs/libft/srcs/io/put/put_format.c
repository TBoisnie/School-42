...
// Declaration -----------------------------------------------------------------
static t_void _format(t_char const * _fmt, va_list * _args, t_state * _state);
...
{
	t_char buffer[FT_IO_FORMAT_BUFFER_SIZE];
	ft_mem_fill(buffer, '\0', FT_IO_FORMAT_BUFFER_SIZE);
...
)
{
	...
	if (ft_chr_is_digit(*_fmt))
		return (ft_str_to_uint_32(_fmt, &_ph->precision));
...
)
{
	if (ft_chr_is_digit(*_fmt))
	{
		...
		return (ft_str_to_uint_32(_fmt, &_ph->width));
	}
...
)
{
	...
	switch (_ph->type)
	{
		case 'd':
		case 'i':
		case 'u': base = FT_STR_INT_BASE_DEC; break;
		case 'x':
		case 'X':
		case 'p': base = FT_STR_INT_BASE_HEX; break;
		case 'o': base = FT_STR_INT_BASE_OCT; break;
		case 'b': base = FT_STR_INT_BASE_BIN; break;
	}

	t_char value_str[64 + 1];
	ft_uint_64_to_str_base(value, value_str, base);
...
{
	t_size position = _state->index % FT_IO_FORMAT_BUFFER_SIZE;

	if (position >= FT_IO_FORMAT_BUFFER_SIZE - 1)
	{
		ft_io_put_str(_state->fd, &_state->buffer[_state->offset]);
		_state->offset = 0;
		ft_mem_fill(_state->buffer, '\0', FT_IO_FORMAT_BUFFER_SIZE);
	}
