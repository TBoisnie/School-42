...
typedef struct
{
	t_fd fd;
	t_size index;
	t_char buffer[FT_GNL_BUFFER_SIZE];
} t_cache;
...
)
{
	...
	{
		if (_cache->index != 0)
			read_len = FT_GNL_BUFFER_SIZE - 1;
		else if ((read_len = read(_cache->fd, _cache->buffer, FT_GNL_BUFFER_SIZE - 1)) < 0)
			return (-1);
...
)
{
	...
	cache->index = 0;
	cache->buffer[0] = '\0';
	cache->buffer[FT_GNL_BUFFER_SIZE - 1] = '\0';
