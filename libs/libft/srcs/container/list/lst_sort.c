...
// )
// {
// 	if (_to && _from != _to && _from != _to->next)
// 	{
// 		t_list * pivot = _partition(_from, _to, _compare, _reverse);
// 		_quick_sort(_from, pivot->prev, _compare, _reverse);
// 		_quick_sort(pivot->next, _to, _compare, _reverse);
// 	}
// }
...
// {
// 	t_list * pivot = _to;
// 	t_list * node = _from->prev;

// 	t_list * current;
// 	for (current = _from; current != _to; current = current->next)
// 	{
// 		t_ssize diff = _compare(current->data, pivot->data);

// 		if ((diff < 0 && !_reverse) || (diff > 0 && _reverse))
// 		{
// 			node = (!node) ? _from : node->next;

// 			ft_lst_swap(node, current);
// 		}
// 	}

// 	node = (!node) ? _from : node->next;
// 	ft_lst_swap(node, _to);
